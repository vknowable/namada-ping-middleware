...
pub async fn all_proposals_handler(query: Query<ProposalQueryParams>, State(app_state): State<Arc<AppState>>) 
  ...
  // find proposal ids to iterate over
  // TODO: add support for pagination options/reverse
  let last_proposal_id_key: Key = governance_storage::get_counter_key();
  let last_proposal_id: u64 = rpc::query_storage_value(app_state.get_client(), &last_proposal_id_key).await?;

  let mut response = ProposalsResponse { proposals: Vec::new(), pagination: None };

  for id in 0..last_proposal_id {
    match get_proposal(app_state.clone(), id, current_epoch, query.proposal_status).await? {
      Some(proposal_item) => response.proposals.push(proposal_item),
      None => continue,
    }
  }
...
async fn get_proposal_result(app_state: Arc<AppState>, id: u64) -> Result<ProposalResult, ApiError> {
